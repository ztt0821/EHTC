...
class HybridTaskCascade(CascadeRCNN):
    ...
    def _mask_forward_train(self,
        ...
        return loss_mask, mask_feats, mask_pred, pos_labels, mask_targets
                    ...
                    with torch.no_grad():
                        ...
                        for j in range(num_imgs):
                            ...
                            sampling_result = bbox_sampler.sample(
                                ...
                                feats=[lvl_feat[j][None] for lvl_feat in x])
                            sampling_results.append(sampling_result)
                loss_mask, mask_feats, mask_pred, pos_labels, mask_targets = self._mask_forward_train(i, x, sampling_results,
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        det_bboxes, det_labels = self.bbox_head[-1].get_det_bboxes(
            ...
            rescale=rescale,
            cfg=rcnn_test_cfg)
        # print(det_bboxes)after softnms
        ...
        if self.with_mask and det_bboxes.shape[0] != 0:
            # get mask scores with mask iou head
            # print(det_labels.size(0))
            # print(det_labels)
            mask_iou_pred = self.mask_iou_head(mask_feats, mask_pred[range(det_labels.size(0)),
                                                                     det_labels + 1])
            mask_scores = self.mask_iou_head.get_mask_scores(
                mask_iou_pred, det_bboxes, det_labels)
            mask_score_all = (ms_segm_result['ensemble'], mask_scores)
            results = (ms_bbox_result['ensemble'], mask_score_all)
        elif self.with_mask and det_bboxes.shape[0] == 0:
